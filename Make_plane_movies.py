...
class Prepare():
    ...
    def load_plane_vectors(self, batch_select = False, select_list = [0,0,0,0], normal_vector_flip = False):
        ...
        # load vectors
        if batch_select == False: # for main_step2
            vector_2C = ff.get_predicted_vectors(os.path.join(patient_path,'vector-pred/batch_'+str(self.batch_i),'pred_2C_t.npy'),os.path.join(patient_path,'vector-pred/batch_'+str(self.batch_i),'pred_2C_r.npy'),scale, image_center)
            vector_3C = ff.get_predicted_vectors(os.path.join(patient_path,'vector-pred/batch_'+str(self.batch_i),'pred_3C_t.npy'),os.path.join(patient_path,'vector-pred/batch_'+str(self.batch_i),'pred_3C_r.npy'),scale, image_center)
            vector_4C = ff.get_predicted_vectors(os.path.join(patient_path,'vector-pred/batch_'+str(self.batch_i),'pred_4C_t.npy'),os.path.join(patient_path,'vector-pred/batch_'+str(self.batch_i),'pred_4C_r.npy'),scale, image_center)
            vector_SA = ff.get_predicted_vectors(os.path.join(patient_path,'vector-pred/batch_'+str(self.batch_i),'pred_BASAL_t.npy'),os.path.join(patient_path,'vector-pred/batch_'+str(self.batch_i),'pred_BASAL_r.npy'),scale, image_center)
        else: # for main_step2c
            print('use selected model set')
            vector_2C = ff.get_predicted_vectors(os.path.join(patient_path,'vector-pred/batch_'+str(select_list[0]),'pred_2C_t.npy'),os.path.join(patient_path,'vector-pred/batch_'+str(select_list[0]),'pred_2C_r.npy'),scale, image_center)
    ...
    def define_SAX_range(self,vector_SA,image_center,normal_vector, seg_LV, batch_index, txt_write = True):
        ...
        print('a is ',a,'b is ',b)
        if txt_write == True:
            t_file = open(os.path.join(self.main_folder,self.patient_class,self.patient_id,"slice_num_info_low_res_batch_"+str(batch_index)+".txt"),"w+")
    ...
    def read_SAX_slice_num_file(self,slice_num_info_file_name):
        slice_num_info_file = open(slice_num_info_file_name, 'r')
        Lines = slice_num_info_file.readlines()
        line1 = Lines[0];line2 = Lines[1]
        num1 = [i for i, e in enumerate(line1) if e == '='][-1]
        a = int(line1[num1+2:len(line1)-1])
        num2 = [i for i, e in enumerate(line2) if e == '='][-1]
        b = int(line2[num2+2:len(line2)])
        return a , b
